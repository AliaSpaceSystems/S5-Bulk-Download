...
```bash
DOWNLOAD_FOLDER_NAME        -f --folder-name                : Path to the folder to store the downloaded products. Leave it empty to use default './downloads'
...
BASELINE                    -b --baseline                   : Processing Baseline
                            -d --dry-run                    : Performs a dry run of the script, creating a csv file output with detailed informations about the products found with the current filters, but without downloading them
...
Usually the user would put into the config file the parameters that do not need to be changed, as the SERVICE_URL, the AUTH_URL, the CLIENT_ID, the DOWNLOAD_FOLDER_NAME and the credentials (USERNAME and PASSWORD). 
